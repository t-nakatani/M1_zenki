...
// #define SIMULATION_DURATION 1 // シミュレーション時間（パケットを発生させる期間）
...
//        show_event_table();
    ...
    void process_event() { // 事象表の先頭事象を取り出して実行
        ...
        {
            Packet *dummy_pkt = new Packet;
            int dst_node_id  = event.pkt->dst_node_id; // d
            int prev_node_id = event.pkt->prev_node_id; // x
            dummy_pkt->dst_node_id  = dst_node_id;
            dummy_pkt->prev_node_id  = prev_node_id;
            double t;
...
//                }
                ...
                t = min_q_value(node_id, dst_node_id); // y(node_id)からd(dst_node_id)に向かう時のyのとりうる最小q値
            }
            ...
            dummy_pkt->prev_node_id = node_id;
            // 前ホップノードの送信完了を事象表に格納
            add_event(current_time, SENDING_FIN, prev_node_id, dummy_pkt, q_value_new);
        ...
        case SENDING_FIN: // パケット送信完了事象の処理
        {
            ...
            for (p = node[prev_node_id].neighbors.begin(); p != node[prev_node_id].neighbors.end(); p++) { 
                ...
                if (p->neighbor_node_id == node_id_){ // yの探索
                    double q_value_old = p->q_value[dst_node_id];
                    p->q_value[dst_node_id] = q_value_new;
                    // cout << q_value_old - q_value_new << endl;
                }
            }
